...
public class DeploymentRepository {
    ...
    public void markRunning(Long id){jdbc.update("UPDATE deployment_task SET status='RUNNING',current_step='PREPARE',start_time=NOW() WHERE id=?",id);}
    ...
    public void image(Long id,String imageTag){jdbc.update("UPDATE deployment_task SET image_tag=? WHERE id=?",imageTag,id);}
    ...
    public long startStep(Long taskId,String code,String name,int sortOrder){
        ...
        jdbc.update(connection->{
            PreparedStatement ps=connection.prepareStatement(
                    ...
                    Statement.RETURN_GENERATED_KEYS);
            ps.setLong(1,taskId);ps.setString(2,code);ps.setString(3,name);ps.setInt(4,sortOrder);return ps;
        },key);
        if(key.getKey()==null)throw new IllegalStateException("step create failed");
        return key.getKey().longValue();
    }

    public void finishStep(Long stepId,String status,String errorMessage){jdbc.update("UPDATE deployment_step SET status=?,error_message=?,end_time=NOW() WHERE id=?",status,errorMessage,stepId);}
    public void skipStep(Long taskId,String code,String name,int sortOrder){jdbc.update("INSERT INTO deployment_step(task_id,step_code,step_name,sort_order,status,start_time,end_time) VALUES(?,?,?,?, 'SKIPPED',NOW(),NOW())",taskId,code,name,sortOrder);}
    ...
    public List<Map<String,Object>> steps(Long taskId){return jdbc.queryForList("SELECT * FROM deployment_step WHERE task_id=? ORDER BY sort_order,id",taskId);}
