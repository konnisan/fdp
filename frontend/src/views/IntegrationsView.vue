...
<template>
  <div class="page-stack restructure-page">
    <PageHeader title="系统集成" description="集中查看 FDP 所在服务器、Docker / Nginx、Codeup 凭据以及云效 Flow / Packages 连接状态。">
    ...
    <div v-if="runtime" class="runtime-summary integration-summary">
      <article class="panel runtime-primary"><div><span>当前开发 / 部署设备</span><h2>{{runtime.os}} · {{runtime.executionMode}}</h2><p>Java {{runtime.javaVersion}} · 预览端口 {{runtime.publicPort}}</p></div><span class="status-text" :class="runtime.liveReady?'running':'pending'"><i></i>{{runtime.liveReady?'LIVE READY':'DRY-RUN SAFE'}}</span></article>
    ...
    <section v-if="runtime" class="panel">
      <div class="panel-head"><div><h2><Server :size="18" />运行依赖</h2><p>Windows 建议保持 FDP_EXECUTION_ENABLED=false；所有 Docker / Nginx 动作进入 DRY-RUN，便于开发验证。</p></div></div>
    ...
    <section class="panel">
      <div class="panel-head"><div><h2><KeyRound :size="18" />Codeup 源码凭据</h2><p>STATIC / LIGHTWEIGHT / CUSTOM 的 HTTPS Codeup 地址通过共享凭据注入，不把 Token 写进 Git URL。</p></div><button class="primary-button" @click="showCredential=true"><Plus :size="14" />新增凭据</button></div>
    ...
    <section class="detail-grid">
      ...
      <article class="panel info-card span-2"><h3><Database :size="17" />平台边界</h3><div class="integration-boundary"><span>Codeup</span><b>源码</b><i>→</i><span>Flow</span><b>CI / Build</b><i>→</i><span>Packages</span><b>Artifact</b><i>→</i><span>FDP</span><b>CD / Runtime</b><i>→</i><span>Docker + Nginx</span><b>客户预览</b></div></article>
    </section>

    <div v-if="showCredential" class="modal-mask" @click.self="showCredential=false"><form class="modal-card" @submit.prevent="saveCredential"><header><div><h2>新增 Codeup 凭据</h2><p>同一账号可复用到该账号有权限访问的多个项目。</p></div><button type="button" class="icon-button" @click="showCredential=false"><X :size="18" /></button></header><div class="form-grid"><label class="span-2">凭据名称 *<input v-model="form.name" /></label><label class="span-2">HTTPS 克隆账号 *<input v-model="form.cloneUsername" /></label><label class="span-2">Personal Access Token *<input v-model="form.token" type="password" autocomplete="new-password" /></label></div><footer><button type="button" class="soft-button" @click="showCredential=false">取消</button><button class="primary-button" :disabled="saving">{{saving?'保存中…':'保存凭据'}}</button></footer></form></div>
  </div>
</template>
