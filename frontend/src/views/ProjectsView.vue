...
import { ExternalLink, FileCode2, KeyRound, Pencil, Plus, RefreshCw, Search, Trash2, X } from 'lucide-vue-next'
...
const defaults = () => ({
  projectCode:'', projectName:'', gitUrl:'', gitBranch:'main', credentialId:null, projectType:'STATIC', projectDirectory:'.',
...
async function load(){
  error.value=''
  try{
    const [ps, cs, rt] = await Promise.all([listProjects(), listSourceCredentials(), getRuntimeStatus()])
    projects.value=ps
...
function openFsStaticCreate(){
  editingId.value=null
  Object.assign(form,defaults(),{
    projectCode:'fs-poc-html',
    projectName:'FS POC HTML',
    gitUrl:'https://codeup.aliyun.com/6038b0d9eb45243512067136/poc-html.git',
    gitBranch:'main',
    projectType:'STATIC',
    projectDirectory:'.',
    buildCommand:'',
    buildOutput:'.',
    previewPath:'/poc',
    credentialId:credentials.value.length===1?credentials.value[0].id:null
  })
  sourceTest.value=null
  showForm.value=true
}
...
async function save(){
  ...
  try{
    const payload={...form,
      buildOutput:form.projectType==='STATIC'?(form.buildOutput?.trim()||'.'):form.buildOutput,
...
<template>
  <div class="page-stack">
    <PageHeader title="交付项目" description="STATIC 直接同步 Codeup 中已经生成好的静态文件；正式工程后续由 Codeup Pipeline 负责构建，FDP 负责获取产物并部署。">
      <template #actions>
        <button class="soft-button" @click="load"><RefreshCw :size="14" />刷新</button>
        <button class="soft-button" @click="openFsStaticCreate"><FileCode2 :size="15" />新建 FS 静态 POC</button>
    ...
    <div v-if="showForm" class="modal-mask" @click.self="showForm=false">
      <form class="modal-card large" @submit.prevent="save">
        <header>
          <div><h2>{{editingId?'编辑交付项目':'新建交付项目'}}</h2><p>STATIC 项目可以直接拉取已经生成好的 HTML；Codeup Token 通过共享凭据注入 Git，不写入仓库 URL。</p></div>
          <button type="button" class="icon-button" @click="showForm=false"><X :size="18" /></button>
        </header>
        <div class="form-grid">
          <label>项目编码 *<input v-model="form.projectCode" placeholder="fs-poc-html" /></label>
          <label>项目名称 *<input v-model="form.projectName" placeholder="FS POC HTML" /></label>
          ...
          <label>交付方式<select v-model="form.projectType"><option value="STATIC">STATIC - 直接静态发布</option><option value="CONTAINER">CONTAINER - 兼容模式</option></select></label>
          ...
          <label class="span-2">仓库内项目目录<input v-model="form.projectDirectory" placeholder=". 表示仓库根目录" /></label>
          <label>访问 Path<input v-model="form.previewPath" placeholder="/poc" /></label>

          <template v-if="form.projectType==='STATIC'">
            <div class="span-2" :style="{fontSize:'12px',color:'#475569',lineHeight:'1.7',padding:'10px 12px',border:'1px solid #dbeafe',background:'#eff6ff'}">
              FS 产出的 poc-html 仓库推荐：仓库内项目目录 = <code>.</code>，构建产物 = <code>.</code>，静态构建命令留空，访问 Path = <code>/poc</code>。这样仓库中的“索通智能工厂/index.html”等目录会整体发布。
            </div>
            <label>构建产物<input v-model="form.buildOutput" placeholder=".（已生成 HTML）或 dist" /></label>
            <label class="span-2">静态构建命令<input v-model="form.buildCommand" placeholder="纯 HTML 留空；需要构建时再填写命令" /></label>
          </template>
