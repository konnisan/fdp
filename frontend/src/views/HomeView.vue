...
const projects=ref([]),deployments=ref([]),loading=ref(true)
const running=computed(()=>projects.value.filter(p=>p.status==='RUNNING'||p.status==='PUBLISHED').length)
...
<template>
  <div class="page-stack">
    <div class="metric-grid">
      <article class="metric-card"><span class="metric-icon blue"><FolderKanban :size="25" /></span><div><p>交付项目总数</p><strong>{{projects.length}}</strong><small>Codeup 驱动</small></div></article>
      <article class="metric-card"><span class="metric-icon green"><Monitor :size="25" /></span><div><p>当前可访问</p><strong>{{running}}</strong><small>STATIC / CONTAINER</small></div></article>
      <article class="metric-card"><span class="metric-icon orange"><Workflow :size="25" /></span><div><p>部署记录</p><strong>{{deployments.length}}</strong><small>Task / Step / Log</small></div></article>
