...
import { Boxes, GitBranch, PackageSearch, RefreshCw, Rocket } from 'lucide-vue-next'
...
function buildFromArtifact(artifact){
  sessionStorage.setItem('fdp-build-from-artifact',JSON.stringify({
    repoId:String(selectedRepository.value?.repoId||''),
    repoName:selectedRepository.value?.repoName||'',
    artifactName:artifact.module||'',
    latestVersion:artifact.versions?.[0]?.version||''
  }))
...
<template>
  <div class="page-stack">
    <PageHeader title="制品仓库" description="直接查看现有云效 Flow 与 Packages 内容，不需要先创建 FDP 项目。找到需要的制品后，可以从这里开始创建 STANDARD 部署项目。">
    ...
    <section class="detail-grid">
      <article class="panel info-card span-2">
        <div class="panel-head"><div><h2><GitBranch :size="18" /> Flow 流水线</h2><p>查看已经由项目经理配置好的构建流水线与运行记录。</p></div><div class="row-actions"><input v-model="pipelineKeyword" placeholder="流水线名称" /><button class="soft-button" @click="loadAll">检索</button></div></div>
      ...
      <article class="panel info-card span-2">
        <div class="panel-head"><div><h2><Boxes :size="18" /> Packages 通用制品仓库</h2><p>直接浏览 Flow 已经上传的可交付产物。</p></div></div>
        <div class="table-wrap"><table class="data-table"><thead><tr><th>仓库</th><th>ID</th><th>类型</th><th>模式</th><th>操作</th></tr></thead><tbody>
          <tr v-for="repo in repositories" :key="repo.repoId"><td><strong>{{repo.repoName}}</strong><small style="display:block">{{repo.repoDesc}}</small></td><td><code>{{repo.repoId}}</code></td><td>{{repo.repoType}}</td><td>{{repo.repoCategory}}</td><td><button class="link-button" @click="openRepository(repo)">查看制品</button></td></tr>
        </tbody></table></div>
      ...
      <article v-if="selectedRepository" class="panel info-card span-2">
        ...
        <div class="table-wrap"><table class="data-table"><thead><tr><th>制品</th><th>组织</th><th>最新版本</th><th>更新时间</th><th>操作</th></tr></thead><tbody>
          <tr v-for="a in artifacts" :key="a.id"><td><strong>{{a.module}}</strong></td><td>{{a.organization||'-'}}</td><td><code>{{a.versions?.[0]?.version||'-'}}</code></td><td>{{time(a.latestUpdate)}}</td><td><button class="primary-button" @click="buildFromArtifact(a)"><Rocket :size="13" />创建 STANDARD</button></td></tr>
        </tbody></table></div>
