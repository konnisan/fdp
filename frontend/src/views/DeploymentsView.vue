...
<template>
  <div class="page-stack">
    <PageHeader title="部署中心" description="借鉴 Jenkins 的任务模型：一次部署对应一个 Task，并记录每个执行步骤和日志。">
    ...
    <section class="panel">
      ...
      <div class="table-wrap"><table class="data-table"><thead><tr><th>ID</th><th>POC 项目</th><th>状态</th><th>当前步骤</th><th>Commit</th><th>开始时间</th><th>操作</th></tr></thead><tbody><tr v-for="d in filtered" :key="d.id"><td>#{{d.id}}</td><td><strong>{{projectName(d.project_id)}}</strong></td><td><span class="tag">{{d.status}}</span></td><td>{{d.current_step}}</td><td><code>{{short(d.commit_id)}}</code></td><td>{{d.start_time||'-'}}</td><td><button class="link-button" @click="show(d)"><Terminal :size="13" />执行详情</button></td></tr></tbody></table></div>
