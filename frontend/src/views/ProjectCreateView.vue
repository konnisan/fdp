...
function deriveSource(){const code=source.projectCode.trim();if(!code)return;if(!source.previewPath)source.previewPath=`/${code}`;if(isContainer.value){if(!source.imageName)source.imageName=`fdp/${code}`;if(!source.containerName)source.containerName=`fdp-${code}`;if(selectedProfile.value==='LIGHTWEIGHT'&&!source.hostDataPath)source.hostDataPath=runtime.value?.dataRoot?`${runtime.value.dataRoot}/${code}`:''}}
