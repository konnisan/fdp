...
import { deployProject, getDeploymentLogs, listDeployments, listProjects, restartProject, stopProject } from '../api'
...
async function openLogs(d){activeTab.value='logs';try{logs.value=await getDeploymentLogs(d.id)}catch(e){error.value=e.response?.data?.message||e.message}}
...
<template>
  <div class="page-stack">
    ...
    <template v-if="project">
      ...
      <section v-if="activeTab==='deployments'" class="panel"><div class="panel-head"><div><h2>发布记录</h2><p>每次 Codeup 同步、构建和发布都会保留记录。</p></div></div><table class="data-table"><thead><tr><th>ID</th><th>状态</th><th>步骤</th><th>Commit</th><th>开始时间</th><th>操作</th></tr></thead><tbody><tr v-for="d in deployments" :key="d.id"><td>#{{d.id}}</td><td><span class="tag">{{d.status}}</span></td><td>{{d.current_step}}</td><td><code>{{short(d.commit_id)}}</code></td><td>{{d.start_time||'-'}}</td><td><button class="link-button" @click="openLogs(d)">查看日志</button></td></tr></tbody></table><div v-if="!deployments.length" class="empty-state">暂无部署记录</div></section>

      <section v-if="activeTab==='logs'" class="panel log-panel"><div class="panel-head"><div><h2><Terminal :size="17" />运行 / 部署日志</h2><p>{{ project.projectType==='STATIC' ? '静态 POC 无常驻 Node.js 进程，以下展示最近部署日志。' : `PM2: ${project.pm2Name||'-'}` }}</p></div><div class="row-actions" v-if="project.projectType==='NODE_SQLITE'"><button class="soft-button" :disabled="busy" @click="act('restart')"><Play :size="14" />重启</button><button class="soft-button" :disabled="busy" @click="act('stop')"><Square :size="14" />停止</button></div></div><pre class="terminal">{{ logs.length ? logs.join('\n\n') : '请选择发布记录中的“查看日志”，或执行一次部署。' }}</pre></section>
    </template>
  </div>
</template>
