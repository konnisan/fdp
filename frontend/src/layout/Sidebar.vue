<script setup>
import { Boxes, ChevronRight, Database, FolderKanban, LayoutDashboard, Workflow } from 'lucide-vue-next'

const props=defineProps({activePath:{type:String,required:true}})
const emit=defineEmits(['navigate'])
const groups=[
  {label:'',items:[{label:'平台总览',path:'/',icon:LayoutDashboard}]},
...
function active(path){
  ...
  if(path==='/projects')return props.activePath.startsWith('/projects')||props.activePath.startsWith('/poc-projects')||['/artifact-delivery','/static-previews','/previews'].includes(props.activePath)
  if(path==='/integrations')return ['/integrations','/yunxiao-artifacts','/runtime'].includes(props.activePath)
...
<template>
  <aside class="sidebar">
    ...
    <div class="sidebar-status"><div class="sidebar-status-title"><Database :size="15" /><span>Deployment Engine</span><b><i></i>Profile V1</b></div><div class="sidebar-status-row"><span>Profiles</span><strong>3 + Custom</strong></div><div class="sidebar-divider"></div><div class="sidebar-status-row"><span>CI / Artifact</span><strong>Flow / Packages</strong></div></div>
  </aside>
</template>
