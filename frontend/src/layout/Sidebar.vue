<script setup>
import { ChevronRight, Database, FolderKanban, LayoutDashboard, Monitor, Workflow } from 'lucide-vue-next'

const props = defineProps({ activePath: { type: String, required: true } })
const emit = defineEmits(['navigate'])
const groups = [
  { label: '', items: [{ label: '平台首页', path: '/', icon: LayoutDashboard }] },
  { label: 'POC 管理', items: [{ label: 'POC 项目', path: '/poc-projects', icon: FolderKanban }, { label: '预览入口', path: '/previews', icon: Monitor }] },
...
<template>
  <aside class="sidebar">
    <button class="brand" type="button" @click="emit('navigate', '/')"><span class="brand-mark"><FolderKanban :size="22" /></span><span><strong>FDP</strong><small>POC部署与预览平台</small></span></button>
    ...
    <div class="sidebar-status"><div class="sidebar-status-title"><Database :size="15" /><span>服务器状态</span><b><i></i>运行中</b></div><div class="sidebar-status-row"><span>外部端口</span><strong>8090</strong></div><div class="sidebar-divider"></div><div class="sidebar-status-row"><span>版本</span><strong>v2.1.0</strong></div></div>
  </aside>
</template>
